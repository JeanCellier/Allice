...
import phenotypage.model.imageEcho.EchoService;
...
import phenotypage.model.ovocytesCollecte.OvocytesCollectesService;
import phenotypage.model.ovocytesCollecte.TableauOvocytesCollectes;
import phenotypage.model.ovocytesCollecte.TableauOvocytesCollectesService;
...
public class OpuController
{
	...
	public String enteteOpu(@PathVariable String nom, Model model)
	{
		...
		if (ficheOpu.getHeader() == null)
		{
			model.addAttribute("header", headerService.newHeader());
		}
		else
		{
			model.addAttribute("header", ficheOpu.getHeader());
		}
	...
	public String enteteOpu(@PathVariable String nom, @Valid Header header, Errors errors, Model model)
	{
		if (errors.hasErrors())
		{
			model.addAttribute("programmeList", programmeService.findAllProgramme());
		...
		if (ficheOpu.getHeader() != null)
		{
			Header headerDeleted = ficheOpu.getHeader();
			ficheOpu.setHeader(null);
		...
		header.setFicheOpu(ficheOpu);
		ficheOpu.setHeader(header);
